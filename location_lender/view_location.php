...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $tableData .= "<tr>";
        $tableData .= "<td>" . $row['location_id'] . "</td>";
        $tableData .= "<td>" . $row['name'] . "</td>";
        $tableData .= "<td>" . $row['price'] . "</td>";
        $tableData .= "<td>" . $row['details'] . "</td>";
        $tableData .= "<td><img  src=../uploads/" . $row['image'] . "></td>";
        $tableData .= "<td>
        <form method='post' >
                    <button type='submit' data-id=". $row['location_id']." class='btn btn-danger' name='delete_location' value=" . $row['location_id'] . ">Delete</button>
                </form>
                </td>";
                
        $tableData .= "<td>
       
       <a href='/film/location_lender/edit_location.php?id=". $row['location_id']."'> <button type='button' id='editbt' data-id=". $row['location_id']." class='btn btn-primary' data-toggle='modal' data-target='#exampleModal'>
        Edit  
      </button></a>
